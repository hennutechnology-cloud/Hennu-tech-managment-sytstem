...
// Value format: "YYYY-MM-DDTHH:mm:ss"  (C# DateTime-compatible)
...
interface DatePickerProps {
  ...
  onChange:     (v: string) => void;  // emits "YYYY-MM-DDTHH:mm:ss"
...
/** Parse both "YYYY-MM-DD" and "YYYY-MM-DDTHH:mm:ss" into a Date. */
function parseDate(v: string): Date | null {
  if (!v) return null;
  // Strip time part if present, keep only date portion for parsing
...
/** Emit a full C# DateTime-compatible ISO 8601 string (midnight, no Z). */
...
/** For "today" shortcut comparison we only need the date portion. */
...
export default function DatePicker({
  ...
  const pick = (day: number) => {
    onChange(toDateTime(new Date(year, month, day)));
    setOpen(false);
  };

  const goToday = () => {
    onChange(toDateTime(new Date()));
  ...
  const todayLabel = lang === "ar"
    ? `اليوم — ${formatDisplay(toDateTime(todayDate), lang)}`
    : `Today — ${formatDisplay(toDateTime(todayDate), lang)}`;
      ...
      {open && (
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
            <button type="button" onClick={isRtl ? handleNext : handlePrev}
              className="p-1.5 hover:bg-white/10 rounded-lg transition-colors text-gray-400 hover:text-white">
              <ChevronLeft className="w-4 h-4" />
            </button>

            <button type="button" onClick={handleHeaderClick}
              className={`flex items-center gap-1.5 px-2 py-1 rounded-lg transition-all duration-150 text-sm font-semibold
                ${view === "years"
                  ? "text-gray-400 cursor-default"
                  : "text-white hover:bg-white/10 hover:text-orange-300"}`}>
              {headerLabel}
              {view !== "years" && (
                <ChevronUp className={`w-3.5 h-3.5 text-orange-400 transition-transform duration-200
                  ${view === "months" ? "rotate-180" : ""}`} />
              )}
            </button>

            <button type="button" onClick={isRtl ? handlePrev : handleNext}
              className="p-1.5 hover:bg-white/10 rounded-lg transition-colors text-gray-400 hover:text-white">
              ...
              <motion.div key="days"
                ...
                <div className="grid grid-cols-7 px-3 pb-3 gap-y-1">
                  {cells.map((day, i) => {
                    ...
                    const thisDp  = `${year}-${String(month + 1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;
                    const isSel   = selected ? datePart(value) === thisDp : false;
                    const isToday = datePart(toDateTime(todayDate)) === thisDp;
                    return (
            ...
            {view === "months" && (
              <motion.div key="months"
                ...
                {monthNames.map((name, i) => {
                  const isCurrent   = selected ? selected.getFullYear() === year && selected.getMonth() === i : false;
                  const isThisMonth = todayDate.getFullYear() === year && todayDate.getMonth() === i;
                  return (
  ...
  return (
    <div className="relative" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <button
        ...
        <span className={`flex-1 text-sm ${value ? "text-white" : "text-gray-500"} ${isRtl ? "text-right" : "text-left"}`}>
