// ============================================================
// InvoiceDetailsModal.tsx  — Full invoice details
// Actions: Edit invoice · Delete invoice · Add/Edit/Delete payments
// ============================================================
import { useState }                from "react";
import { motion, AnimatePresence }  from "motion/react";
import {
  X, CreditCard, FileDown, TrendingUp, TrendingDown,
  Building2, Users, Calendar, CheckCircle2, Edit2, Trash2,
} from "lucide-react";
import DeleteConfirmModal from "./DeleteConfirmModal";
import {
  tInv, formatCurrency, formatDate,
  STATUS_STYLES, ACCOUNT_STYLES,
...
export default function InvoiceDetailsModal({
  ...
  // Local delete-confirm state for invoice
  const [confirmInvoice,   setConfirmInvoice]   = useState(false);
  // Local delete-confirm state for a payment
  ...
  const pct  = invoice.totalAmount > 0
    ...
    : 0;
  const pColor =
    pct >= 100 ? "bg-emerald-500" : pct >= 60 ? "bg-blue-500" : pct > 0 ? "bg-orange-500" : "bg-red-500/50";
              ...
              >
                ...
                <div className={`flex items-start justify-between px-5 py-4 sm:px-6
                  ...
                  <div className={`min-w-0 ${isRtl ? "text-right" : ""}`}>
                    ...
                    <p className="font-mono text-xs text-orange-400/80 mt-0.5">{invoice.invoiceNumber}</p>
                  </div>
                        ...
                        {invoice.payments.map((pay) => (
                          <div key={pay.id}
                            ...
                            <div className={`flex-1 min-w-0 ${isRtl ? "text-right" : ""}`}>
                              ...
                              {pay.note && (
                                <p className="text-[10px] text-gray-600 mt-0.5 italic">{pay.note}</p>
                              )}
                            </div>

                            {/* Payment actions — visible on hover */}
                            <div className={`flex items-center gap-1 shrink-0
                                             opacity-0 group-hover:opacity-100 transition-opacity
                                             ${isRtl ? "flex-row-reverse" : ""}`}>
                              <button
                                onClick={() => onEditPayment(invoice, pay)}
                                className="p-1.5 rounded-lg hover:bg-blue-500/15 text-gray-500 hover:text-blue-400 transition-colors"
                                title={tInv(lang, "editPayment")}
                              >
                                <Edit2 className="w-3.5 h-3.5" />
                              </button>
                              <button
                                onClick={() => setConfirmPaymentId(pay.id)}
