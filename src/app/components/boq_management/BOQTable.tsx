// ============================================================
// BOQTable.tsx — Responsive
// Mobile  (<md):  card-per-item layout stacked vertically
// Tablet  (md):   condensed table (unit/qty/price visible, actions on hover)
// Desktop (lg+):  full table with all columns
// ============================================================
import { motion, AnimatePresence } from "motion/react";
import { ChevronDown, ChevronRight, ChevronLeft, Edit, Trash2, TrendingUp, TrendingDown } from "lucide-react";
...
import type { BOQTableProps, BOQItem, BOQSection } from "../../core/models/BOQManagement.types";
import type { Lang } from "../../core/models/Settings.types";

// ── helpers ───────────────────────────────────────────────────
const flip = (lang: Lang, ltr: string, rtl: string) =>
  lang === "ar" ? rtl : ltr;

// ── Mobile Item Card ──────────────────────────────────────────
function ItemCard({
  item, showComparison, onEdit, onDelete, lang,
}: {
  item:           BOQItem;
...
function SectionBlock({
  section, expanded, onToggle, showComparison, onEdit, onDelete, lang,
}: {
  section:        BOQSection;
  expanded:       boolean;
  onToggle:       () => void;
      ...
      >
        <span className="text-sky-400 shrink-0">
          {expanded
            ...
            : <CollapsedChevron className="w-4 h-4" />}
        </span>
...
function SectionRow({
  ...
  return (
    ...
    >
      ...
      <td
        colSpan={colSpan}
        className={`py-3 px-4 font-semibold text-white text-[15px] ${flip(lang, "text-left", "text-right")}`}
      >
        <div className={`flex items-center gap-2 ${isRtl ? "flex-row-reverse" : ""}`}>
          {section.name}
...
// ── Table Item Row (md+) ──────────────────────────────────────
function ItemRow({
  item, isLast, showComparison, onEdit, onDelete, lang,
}: {
  item:            BOQItem;
  isLast:          boolean;
  showComparison:  boolean;
  ...
  return (
    ...
    >
        ...
        <div className={`flex items-center ${isRtl ? "flex-row-reverse" : ""}`}>
          <div className="w-5 shrink-0 flex justify-center relative" style={{ marginInlineStart: "20px" }}>
            ...
            <div
              ...
              style={{ width: "10px", [isRtl ? "left" : "right"]: 0, top: "50%" }}
            />
          </div>
      ...
      {/* Unit price — hidden on md, shown on lg */}
      ...
      {/* Month — hidden on md, shown on lg */}
      <td className="hidden lg:table-cell py-3 px-4 text-sm text-gray-400 text-center whitespace-nowrap">
        {resolveMonth(lang, item.month)}
      </td>

      {/* Actual + variance (comparison mode) */}
      {showComparison && (
        <>
          <td className={`py-3 px-4 text-sm font-semibold text-white whitespace-nowrap ${flip(lang, "text-right", "text-left")}`}>
...
export default function BOQTable({
  ...
  onToggleSection, onEdit, onDelete, lang,
}: BOQTableProps) {
  const isRtl          = lang === "ar";
  ...
  // Tablet (md): code + description + qty + estimated + [actual + variance?] + actions = 5 or 7
  // Desktop (lg): + unit + unitPrice + month = 8 or 10
  const extraColSpan       = showComparison ? 2 : 0;
  // lg colspan (all cols): 7 visible cols + extras
  const lgSectionNameSpan  = 6 + extraColSpan;
  // md colspan (fewer cols): code + description + qty + estimated + [comparison?] + actions → name spans 3 + extras
  const mdSectionNameSpan  = 3 + extraColSpan;
  ...
  return (
    <GlassCard className="overflow-hidden">
      ...
      <div className="md:hidden p-3 flex flex-col gap-3" dir={isRtl ? "rtl" : "ltr"}>
        ...
        {/* Mobile Grand Total */}
        <div
          className={`flex items-center justify-between bg-white/[0.04] border border-white/10 rounded-xl px-4 py-3 mt-1 ${isRtl ? "flex-row-reverse" : ""}`}
        >
        ...
        <table className="w-full" dir={isRtl ? "rtl" : "ltr"}>
          <thead>
            <tr className="border-b border-white/10">
              <th className={`py-4 px-4 text-xs font-semibold text-gray-400 whitespace-nowrap ${thAlign}`}>
                {tBOQMgt(lang, "colCode")}
              </th>
              <th className={`py-4 px-4 text-xs font-semibold text-gray-400 whitespace-nowrap ${thAlign}`}>
                {tBOQMgt(lang, "colDescription")}
              </th>
              {/* Unit — desktop only */}
              <th className={`hidden lg:table-cell py-4 px-4 text-xs font-semibold text-gray-400 text-center whitespace-nowrap`}>
                {tBOQMgt(lang, "colUnit")}
              </th>
              <th className={`py-4 px-4 text-xs font-semibold text-gray-400 whitespace-nowrap ${flip(lang, "text-right", "text-left")}`}>
                {tBOQMgt(lang, "colQuantity")}
              </th>
              {/* Unit price — desktop only */}
              <th className={`hidden lg:table-cell py-4 px-4 text-xs font-semibold text-gray-400 whitespace-nowrap ${flip(lang, "text-right", "text-left")}`}>
                {tBOQMgt(lang, "colUnitPrice")}
              </th>
              <th className={`py-4 px-4 text-xs font-semibold text-gray-400 whitespace-nowrap ${flip(lang, "text-right", "text-left")}`}>
                {tBOQMgt(lang, "colEstimated")}
              </th>
              {/* Month — desktop only */}
              <th className="hidden lg:table-cell py-4 px-4 text-xs font-semibold text-gray-400 text-center whitespace-nowrap">
                {tBOQMgt(lang, "colMonth")}
              ...
              {showComparison && (
                <>
                  ...
                  <th className="py-4 px-4 text-xs font-semibold text-gray-400 text-center whitespace-nowrap">
                    {tBOQMgt(lang, "colVariance")}
                  </th>
                </>
              )}
              <th className={`py-4 px-4 text-xs font-semibold text-gray-400 whitespace-nowrap ${flip(lang, "text-right", "text-left")}`}>
                {tBOQMgt(lang, "colActions")}
              </th>
            </tr>
          </thead>
          ...
          <tbody>
            {sections.map((section) => {
              ...
              // Use lg colspan normally; on md, fewer cols are visible
              // We pass the lg span; on md the hidden cols collapse naturally
              return (
          ...
          <tfoot className="border-t-2 border-white/20 bg-white/[0.03]">
            <tr>
              {/* Spans: code + description + qty + estimated = 4 cols on md, plus hidden cols on lg */}
              <td
                colSpan={4}
                className={`px-4 py-4 font-bold text-white text-sm ${flip(lang, "text-right", "text-left")}`}
              >
                {tBOQMgt(lang, "tableTotal")}
              </td>
              {/* Hidden spacers for lg-only columns */}
