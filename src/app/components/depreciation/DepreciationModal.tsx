...
function validate(values: DepreciationFormValues): DepreciationFormErrors {
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* Modal — NO overflow-hidden so DatePicker portal is never clipped */}
          <motion.div
            key="modal"
          ...
          >
            <div className="w-full max-w-lg bg-[#0f1117] border border-white/10 rounded-2xl shadow-2xl shadow-black/50">
              ...
              <div className="flex items-center justify-between px-6 py-5 border-b border-white/10 bg-gradient-to-l from-orange-500/10 to-transparent rounded-t-2xl">
                <div>
                  <h2 className="text-lg font-bold text-white">
                    {isEdit ? "تعديل الأصل" : "إضافة أصل جديد"}
                  </h2>
                  <p className="text-xs text-gray-400 mt-0.5">
                    {isEdit
                      ? `تعديل بيانات ${editAsset!.name}`
              ...
              <div className="px-6 py-6 space-y-5">
                ...
                <Field label="اسم الأصل" error={errors.name}>
                  <input
                    type="text"
                    placeholder="مثال: مبنى المكتب"
                    value={values.name}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="التكلفة (ر.س)" error={errors.cost}>
                    <input
                  ...
                  <Field label="القيمة المتبقية (ر.س)" error={errors.salvageValue}>
                    <input
                      type="number" dir="ltr" placeholder="0" min="0"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="العمر الإنتاجي (سنة)" error={errors.usefulLife}>
                    <input
                      type="number" dir="ltr" placeholder="10" min="1"
                  ...
                  <Field label="طريقة الإهلاك">
                    ...
                    >
                      <option value="قسط ثابت"   className="bg-[#0f1117]">قسط ثابت</option>
                      <option value="قسط متناقص" className="bg-[#0f1117]">قسط متناقص</option>
                    </select>
                  </Field>
                </div>

                {/* Shared DatePicker — portal renders into document.body so never clipped */}
                <DatePicker
                  label="تاريخ الشراء"
                ...
                <AnimatePresence>
                  {confirmDelete && (
                    ...
                    >
                      هل أنت متأكد من حذف هذا الأصل؟ اضغط حذف مرة أخرى للتأكيد.
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
