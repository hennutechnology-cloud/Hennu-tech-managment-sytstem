// ============================================================
// IncomeNoDataAlert.tsx — Responsive
// ============================================================
...
export default function IncomeNoDataAlert({ isOpen, onClose, lang }: NoDataAlertProps) {
  const innerContent = (
    <>
      ...
      <h3 className="text-base sm:text-lg font-bold text-white mb-2">{tIS(lang, "noDataTitle")}</h3>
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.div key="inc-backdrop"
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            onClick={onClose}
            className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50" />

          {/* Mobile: bottom sheet */}
          <motion.div key="inc-mobile"
